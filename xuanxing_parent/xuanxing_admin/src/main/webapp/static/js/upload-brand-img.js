...
var uploaderBrandLogo = new plupload.Uploader({
	runtimes : 'html5,flash,silverlight,html4',
	browse_button : 'brandselectfiles', 
    //runtimes : 'flash',
	...
	init: {
		FilesAdded: function(up, files) {
			plupload.each(files, function(file) {
				...
				$("#minfileName").html(oldFileName);
				$("#minfileId").val(basePath+file.name);
			});
		},
		 //上传之前执行的函数
		...
		FileUploaded: function(up, file, info) {
            ...
            {
            	//上传成功后续操作
            	uploadBrandSucess(osssignature.host,file.name);
            }
            else
            {
            	bootbox.alert("上传失败");
